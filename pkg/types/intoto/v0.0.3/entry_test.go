...
func TestNewEntryReturnType(t *testing.T) {
	entry := NewEntry()
	if reflect.TypeOf(entry) != reflect.ValueOf(&V003Entry{}).Type() {
...
func createRekorEnvelope(dsseEnv *dsse.Envelope, pub [][]byte) *models.IntotoV003SchemaProposedContent {

	envelopeBytes, _ := json.Marshal(dsseEnv)
	env := &models.IntotoV003SchemaProposedContent{
		Envelope: swag.String(string(envelopeBytes)),
	}
	for _, key := range pub {
		env.PublicKeys = append(env.PublicKeys, strfmt.Base64(key))
	}
	return env
}

func TestV003Entry_Unmarshal(t *testing.T) {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			var uv = func() error {
				...
				if !reflect.DeepEqual(v.AttestationKey(), "sha256:"+hex.EncodeToString(h[:])) {
					t.Errorf("V003Entry.AttestationKey() = %v, want %v", v.AttestationKey(), "sha256:"+hex.EncodeToString(h[:]))
				}
				...
				if !reflect.DeepEqual(got, want) {
					t.Errorf("V003Entry.IndexKeys() = %v, want %v", got, want)
				}
				...
				canonicalV003 := canonicalEntry.(*V003Entry)
				if *canonicalV003.IntotoObj.EnvelopeHash.Value != envHash {
					t.Errorf("envelope hashes do not match post canonicalization: %v %v", *canonicalV003.IntotoObj.EnvelopeHash.Value, envHash)
				}
				if canonicalV003.AttestationKey() != "" && *canonicalV003.IntotoObj.PayloadHash.Value != payloadHash {
					t.Errorf("payload hashes do not match post canonicalization: %v %v", canonicalV003.IntotoObj.PayloadHash.Value, payloadHash)
				}
				canonicalIndexKeys, _ := canonicalV003.IndexKeys()
			...
			if err := uv(); (err != nil) != tt.wantErr {
				t.Errorf("V003Entry.Unmarshal() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestV003Entry_IndexKeys(t *testing.T) {
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			envHashBytes := sha256.Sum256([]byte(*v.IntotoObj.ProposedContent.Envelope))
			envHash := hex.EncodeToString(envHashBytes[:])
